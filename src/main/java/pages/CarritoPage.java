...
public class CarritoPage extends BasePage {
    ...
    private By tablaProductos = By.xpath("//form[contains(@action, 'cart/edit')]//table");
    private By filasProductos = By.xpath("//form[contains(@action, 'cart/edit')]//table//tbody/tr");
    ...
    private By mensajeCarritoVacio = By.xpath("//p[contains(text(),'Your shopping cart is empty')]");

    public void irAlCarrito() {
        WaitUtils.esperarElementoClickable(driver, botonCarrito, 10).click();
    }

    public boolean estaCarritoVacio() {
        // Verifica si aparece el mensaje de "Your shopping cart is empty!"
        List<WebElement> mensaje = driver.findElements(mensajeCarritoVacio);

        return !mensaje.isEmpty(); // Si hay mensaje, está vacío
    }
